...
class App(object):
    ...
    def execute(self, data: TrajectoryCollection, config: dict) -> TrajectoryCollection:
        print("Executing...")
        start = time.time()
        geoframe, col_name = mv.convertToGeoPandasFrame(data)
        qgs = mv.qgsAppInit()
        ...
        mv.produceOutputs(intersected_gpd, self.moveapps_io, col_name)
        trajAltered = mv.gpdToMpd(intersected_gpd, col_name)
        mv.qgsAppExit(qgs)
        end = time.time()
